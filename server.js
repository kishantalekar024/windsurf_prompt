...
const MONGO_URI =
  ...
  "mongodb+srv://windsurf_prompt:Ug7vl28TQok7yJkQ@cluster0.fqpyy.mongodb.net/?appName=Cluster0";
...
app.get("/prompts", async (req, res) => {
  try {
    ...
    if (isConnected && collection) {
      ...
      const prompts = allPrompts.slice(skip, skip + limit).map((p) => ({
  ...
  try {
    if (isConnected && collection) {
      ...
      const responseStats = {
        ...
        brain_enabled_percentage:
          stats.total_prompts > 0
            ? Math.round(
                (stats.brain_enabled_count / stats.total_prompts) * 100,
              )
            : 0,
      ...
      allPrompts.forEach((prompt) => {
        ...
        // Hourly distribution - handle both string and Date timestamps
        if (prompt.timestamp) {
          const timestamp =
            typeof prompt.timestamp === "string"
              ? new Date(prompt.timestamp)
      ...
      const responseStats = {
        ...
        avg_prompt_length:
          prompts.length > 0
            ? Math.round(totalPromptLength / prompts.length)
            : 0,
        brain_enabled_percentage:
          prompts.length > 0
